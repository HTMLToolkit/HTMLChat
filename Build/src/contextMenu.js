...
export class ContextMenuManager {
  ...
  injectIcons() {
    // Replace every .context-icon with the proper SVG
    this.menu.querySelectorAll('.context-item').forEach(item => {
      const iconElem = item.querySelector('.context-icon');
      if (iconElem && iconElem.dataset.lucide) {
        const iconName = iconElem.dataset.lucide;
        // Replace placeholder <i> with SVG
        iconElem.outerHTML = createIconHTML(iconName, { class: 'context-icon', size: 14 });
      }
    });
  }
    ...
    if (reason !== null) { // null means cancelled
      try {
        const res = await fetch(`${this.app.baseURL}/mod/${this.app.elements.roomSelect.value}?user=${encodeURIComponent(this.app.user)}`, {
          method: "POST",
          headers: this.app.getAuthHeaders(true),
          body: JSON.stringify({
            action: 'kick',
